...
export default function Doar() {
  ...
  const [creators, setCreators] = useState<Criador[]>([])
  ...
  const loadCreators = async () => {
    try {
      ...
      if (response.ok) {
        setCreators(data.dados?.criadores || [])
        console.log('Criadores carregados:', data.dados?.criadores?.length || 0)
      } else {
  ...
  const filteredCreators = creators.filter(creator => {
    ...
    console.log('Usuário atual:', usuario?.nome, 'Criador ID:', usuario?.criador?.id)
    
    // Excluir o próprio usuário se ele for um criador
    if (usuario?.criador && usuario.criador.id === creator.id) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (usuario.criador && usuario.criador.id === selectedCreator) {
      setNotification({ type: 'error', message: 'Você não pode doar sementes para si mesmo!' })
      return
    }

    const valorNum = parseInt(valor)
    if (valorNum > usuario.sementes) {
    ...
    try {
      ...
      if (response.ok) {
        // Atualizar sementes do usuário no localStorage
        const usuarioAtualizado = { ...usuario, sementes: usuario.sementes - valorNum }
                      ...
                      >
                        <div className="flex items-start space-x-3">
                          ...
                          <div className="flex-1 min-w-0 text-left">
                            ...
                            {/* Redes Sociais */}
                            {(creator.redesSociais?.youtube || creator.redesSociais?.twitch || creator.redesSociais?.instagram || creator.redesSociais?.tiktok || creator.redesSociais?.discord) && (
